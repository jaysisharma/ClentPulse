...
export function ClientsView({ clients }: { clients: ClientCard[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-11 pl-10 pr-4 rounded-xl bg-white border border-slate-200 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-colors"
          />
        </div>
          ...
          {FILTERS.map(f => (
            <button
              ...
              className={cn(
                ...
                filter === f.key ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-900',
              )}
            >
              {f.label}
            </button>
      ...
      {/* Results */}
      {filtered.length === 0 ? (
        <div className="rounded-2xl bg-white border border-slate-200 py-14 px-6 text-center">
          <div className="w-11 h-11 bg-slate-100 rounded-xl flex items-center justify-center mx-auto mb-3">
            <Users className="w-5 h-5 text-slate-400" />
          </div>
          <p className="text-sm font-semibold text-slate-700">No clients match</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filtered.map(client => (
            <div
              key={client.key}
              className="rounded-2xl bg-white border border-slate-200 p-5 hover:border-slate-300 transition-colors flex flex-col"
            >
              {/* Identity */}
              <div className="flex items-start gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {client.email ? (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-indigo-600 transition-colors mt-0.5 max-w-full"
                    >
                ...
                {client.email ? (
                  ...
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full flex-shrink-0">
              ...
              <div className="flex items-center gap-4 mt-4 text-xs font-medium text-slate-500">
                <span className="inline-flex items-center gap-1.5">
                  ...
                  <span className="text-slate-900 font-bold tabular-nums">{client.projectCount}</span>
                  project{client.projectCount !== 1 ? 's' : ''}
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <Mail className="w-3.5 h-3.5 text-slate-400" />
                  <span className="text-slate-900 font-bold tabular-nums">{client.sentUpdates}</span>
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 flex flex-wrap gap-2">
                {client.projects.map(p => (
                  <Link
                    key={p.id}
                    href={`/project/${p.id}`}
                    className="inline-flex items-center gap-2 text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 hover:border-indigo-300 hover:text-indigo-600 transition-colors"
                  >
                ...
                <Link
                  href="/project/new"
                  className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-indigo-600 transition-colors px-2 py-1.5"
                >
