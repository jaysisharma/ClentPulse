        ...
        {total > 0 && (
          <div className="space-y-1">
            <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400">
              ...
              <span className="font-mono">{Math.round((totalDone / total) * 100)}%</span>
            </div>
...
function Section({
  ...
  const accent = color === 'indigo'
    ...
    : 'text-violet-700 dark:text-violet-400 bg-violet-500/10 border border-violet-500/20'
  const doneColor = color === 'indigo' ? '#6366F1' : '#8B5CF6'

  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <div className={`inline-flex items-center gap-1.5 text-[11px] font-semibold tracking-wider px-2.5 py-0.5 rounded-full ${accent}`}>
          {icon}
          <span>{label}</span>
          {items.length > 0 && (
            <span className="font-mono text-[10px] opacity-80">· {items.filter(i => i.done).length}/{items.length}</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onAdd}
            className="text-xs text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors flex items-center gap-1"
          >
      ...
      {/* Template pills */}
      {showTemplates && (
        <div className="flex flex-wrap gap-1.5 mb-3 p-3 bg-slate-50 dark:bg-white/[0.03] rounded-xl border border-slate-200 dark:border-white/10 animate-fade-in">
          {templates.map(t => (
            <button
              key={t}
              onClick={() => onPickTemplate(t)}
              className="text-xs text-slate-700 dark:text-slate-300 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full px-2.5 py-1 hover:border-indigo-400 dark:hover:border-indigo-400 hover:text-slate-900 dark:hover:text-white transition-colors text-left shadow-xs"
            >
              {t}
            </button>
          ))}
      ...
      <div className="space-y-1.5">
        {items.length === 0 && !adding ? (
          <p className="text-xs text-slate-400 dark:text-slate-500 py-1 italic font-light">{emptyText}</p>
        ) : (
          items.map(item => (
            <div key={item.id} className="flex items-center gap-2.5 group py-0.5">
              <button onClick={() => onToggle(item)} className="flex-shrink-0">
                {item.done
                  ...
                  : <Circle className="w-4 h-4 text-slate-300 dark:text-slate-600 hover:text-slate-500 transition-colors" />
                }
              </button>
              <span className={`flex-1 text-xs sm:text-sm min-w-0 ${item.done ? 'line-through text-slate-400 dark:text-slate-500' : 'text-slate-800 dark:text-slate-200'}`}>
                {item.title}
              </span>
              {item.done_at && (
                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono flex-shrink-0 hidden group-hover:block">
        ...
        {adding && (
          <div className="flex gap-2 mt-2">
            ...
            <button
              ...
              className="px-3.5 py-1.5 text-xs font-semibold rounded-full bg-slate-900 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 disabled:opacity-50 transition-all shadow-xs"
            >
              Add
            </button>
            <button
              onClick={onCancel}
              className="px-2 py-1.5 text-xs text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors"
            >
              Cancel
            </button>
          </div>
