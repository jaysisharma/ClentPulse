  ...
  return (
    <div className="space-y-4">
      {visibleUpdates.map(update => (
        <div key={update.id} className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 shadow-sm rounded-2xl p-6 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              {update.sent_at ? (
                ...
                <span className="text-xs font-semibold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-full font-sans">Draft</span>
              )}
          ...
          <ul className="space-y-2.5">
            {(update.bullets ?? []).filter(Boolean).map((b: string, i: number) => (
              <li key={i} className="flex items-start gap-2.5 text-sm text-slate-700 dark:text-slate-200 leading-relaxed font-sans">
          ...
          {update.note && (
            <p className="text-sm text-slate-500 dark:text-slate-400 italic border-t border-slate-100 dark:border-slate-800 pt-3 mt-4 font-sans">{update.note}</p>
          )}
          <UpdateCommentForm
            updateId={update.id}
            projectId={projectId}
      ...
      {visibleCount < updates.length && (
        <button
          ...
          className="w-full py-3 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-indigo-600 bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 rounded-2xl hover:bg-slate-100 dark:bg-slate-800 transition-colors mt-4 cursor-pointer font-sans"
        >
