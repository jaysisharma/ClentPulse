  ...
  return (
    <AppLayout>
      <div className="max-w-2xl animate-fade-in">
        <Link href={`/invoices/${id}`} className="inline-flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 mb-6 transition-colors">
          <ArrowLeft className="w-4 h-4" />Back to invoice
        </Link>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">Edit invoice</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">Changes are saved immediately. Status is not affected.</p>
        ...
        <div className="space-y-5">
          ...
          <div className="flex gap-3">
            <Link href={`/invoices/${id}`} className="flex-1">
              <Button variant="secondary" className="w-full justify-center">Cancel</Button>
            </Link>
            <Button onClick={handleSave} loading={loading} disabled={!valid} className="flex-1 justify-center">
              Save changes
            </Button>
          </div>
        </div>
