      ...
      <div className="max-w-6xl mx-auto space-y-16 relative z-10">
        ...
        <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {testimonials.map((t) => (
            <div
              key={t.name}
              className="p-7 rounded-3xl bg-white border border-slate-200/80 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] flex flex-col justify-between space-y-6 hover:shadow-xl hover:border-slate-300 hover:-translate-y-1.5 transition-all duration-300 relative group"
            >
              {/* Star Rating */}
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-slate-200/60">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs ${t.badgeColor} group-hover:scale-110 transition-transform shadow-sm`}>
                  {t.initials}
                </div>
                <div>
